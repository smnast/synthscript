...
std::shared_ptr<Object> BuiltinFunctions::built_in_len(std::vector<std::shared_ptr<Object>> *arguments, int line, int col) {
    ...
    if (arguments->at(0)->get_type() == TYPE_ARRAY) {
        return std::make_shared<IntObject>(std::static_pointer_cast<ArrayObject>(arguments->at(0))->get_len());
    } else {
        return std::make_shared<IntObject>(std::static_pointer_cast<StringObject>(arguments->at(0))->get_len());
    }
}
