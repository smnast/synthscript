...
#define BUILT_IN_FUNCTION(name, param_count)                                                       \
    {                                                                                              \
        #name, {                                                                                   \
            [](std::vector<std::shared_ptr<Object>> arguments, int line,                           \
...
std::unordered_map<std::string, BuiltinFunction> BuiltinFunctions::built_in_functions = {
    BUILT_IN_FUNCTION(output, 1), BUILT_IN_FUNCTION(input, 0),
    BUILT_IN_FUNCTION(read, 1),   BUILT_IN_FUNCTION(write, 2),
    BUILT_IN_FUNCTION(append, 2), BUILT_IN_FUNCTION(current_directory, 0),
    BUILT_IN_FUNCTION(len, 1),    BUILT_IN_FUNCTION(sum, 1),
...
std::shared_ptr<Object>
BuiltinFunctions::built_in_output(std::vector<std::shared_ptr<Object>> *arguments, int line,
                                  int col) {
    std::shared_ptr<Object> cast_obj = arguments->at(0)->cast(TYPE_STRING);
    if (cast_obj == nullptr) {
        Error::runtime_error("Invalid argument to builtin output of type " +
...
std::shared_ptr<Object>
BuiltinFunctions::built_in_input(std::vector<std::shared_ptr<Object>> *arguments, int line,
                                 int col) {
    std::string input;
    std::cin >> input;
...
std::shared_ptr<Object>
BuiltinFunctions::built_in_read(std::vector<std::shared_ptr<Object>> *arguments, int line,
                                int col) {
    std::shared_ptr<Object> file_path_obj = arguments->at(0)->cast(TYPE_STRING);
    if (file_path_obj == nullptr) {
        Error::runtime_error("Invalid argument to builtin read of type " +
                                 type_strings[arguments->at(0)->get_type()],
                             line, col);
    }
...
std::shared_ptr<Object>
BuiltinFunctions::built_in_write(std::vector<std::shared_ptr<Object>> *arguments, int line,
                                 int col) {
    std::shared_ptr<Object> file_path_obj = arguments->at(0)->cast(TYPE_STRING);
    if (file_path_obj == nullptr) {
        Error::runtime_error("Invalid argument to builtin write of type " +
                                 type_strings[arguments->at(0)->get_type()],
                             line, col);
    }
    std::string file_path = std::static_pointer_cast<StringObject>(file_path_obj)->get_value();
...
std::shared_ptr<Object>
BuiltinFunctions::built_in_append(std::vector<std::shared_ptr<Object>> *arguments, int line,
                                  int col) {
    std::shared_ptr<Object> file_path_obj = arguments->at(0)->cast(TYPE_STRING);
    if (file_path_obj == nullptr) {
        Error::runtime_error("Invalid argument to builtin append of type " +
                                 type_strings[arguments->at(0)->get_type()],
                             line, col);
    }
    std::string file_path = std::static_pointer_cast<StringObject>(file_path_obj)->get_value();
...
std::shared_ptr<Object>
BuiltinFunctions::built_in_current_directory(std::vector<std::shared_ptr<Object>> *arguments,
                                             int line, int col) {
    char cwd[1024];
...
BuiltinFunctions::built_in_len(std::vector<std::shared_ptr<Object>> *arguments, int line, int col) {
    if (arguments->at(0)->get_type() != TYPE_ARRAY && arguments->at(0)->get_type() != TYPE_STRING) {
        Error::runtime_error("Invalid argument to builtin len of type " +
                                 type_strings[arguments->at(0)->get_type()],
                             line, col);
    }
...
BuiltinFunctions::built_in_sum(std::vector<std::shared_ptr<Object>> *arguments, int line, int col) {
    if (arguments->at(0)->get_type() != TYPE_ARRAY) {
        Error::runtime_error("Invalid argument to builtin sum of type " +
                                 type_strings[arguments->at(0)->get_type()],
                             line, col);
    }
...
std::shared_ptr<Object>
BuiltinFunctions::built_in_product(std::vector<std::shared_ptr<Object>> *arguments, int line,
                                   int col) {
    if (arguments->at(0)->get_type() != TYPE_ARRAY) {
        Error::runtime_error("Invalid argument to builtin product of type " +
                                 type_strings[arguments->at(0)->get_type()],
                             line, col);
    }
